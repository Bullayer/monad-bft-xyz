...
impl ChainStateBuffer {
    ...
    pub async fn insert(&self, block_event: EventServerEvent) {
        ...
        if let Some(old_block) = self.by_height.insert(block_height, block) {
            warn!(
                ?block_height,
            ...
            self.by_hash.remove(&FixedData(old_block.header.hash.0));

            // Clean up old block's transactions from the transactions map
            match old_block.transactions {
                ...
                alloy_rpc_types::BlockTransactions::Hashes(_) => {
                    error!("ChainStateBuffer replaced block transactions contained hashes");
                }
                alloy_rpc_types::BlockTransactions::Uncle => {
                    error!("ChainStateBuffer replaced block transactions were uncle");
                }
            }
        }
        if self
...
mod tests {
    ...
    async fn test_many_proposed_blocks() {
        ...
        for i in 0..total_blocks {
            ...
            let ring = buffer.ring.lock().await;
            ...
            if i >= capacity {
                ...
                assert!(buffer.by_height.get(&evicted_height).is_none(),
                    "Block at height {} should be evicted", evicted_height);
            }
            assert!(buffer.by_height.get(&height).is_some(),
        ...
        for i in 0..total_blocks {
            ...
            if i < total_blocks - capacity {
                ...
                assert!(buffer.by_height.get(&height).is_none(),
    ...
    async fn test_duplicate_height_different_hash() {
        ...
        assert_eq!(buffer.by_height.len(), 1, "Should have 1 entry in by_height after first insert");
        assert_eq!(buffer.by_hash.len(), 1, "Should have 1 entry in by_hash after first insert");
        assert_eq!(
            buffer.by_height.len(),
            buffer.by_hash.len(),
        ...
        let stored_block = buffer.by_height.get(&height).expect("Block should exist at height 1");
        assert_eq!(
        ...
        let hash_a_exists = buffer.by_hash.get(&FixedData(block_hash_a.0)).is_some();
        let hash_b_exists = buffer.by_hash.get(&FixedData(block_hash_b.0)).is_some();

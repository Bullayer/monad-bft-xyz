...
use alloy_primitives::{b256, keccak256, Address, FixedBytes, B256};
use alloy_rlp::{
...
impl Decodable for AccountCodeOrHash {
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        let account: AccountCodeOrHash = match buf.len() {
            ...
            RLP_HASH_LENGTH => {
                match <[u8; HASH_LENGTH]>::decode(buf) {
                    Ok(x) => {
                        // validate data
                        let res: Result<B256, _> = keccak256(x).as_slice().try_into();
                        match res {
                            Ok(_) => {}
                            Err(e) => {
                                warn!("validation error extracting code_hash {:?}", e);
                                return Err(alloy_rlp::Error::Custom("error extracting code_hash"));
                            }
                        }

                        AccountCodeOrHash::CodeHash(B256::from(x))
                    }
                    Err(e) => {
...
impl AccountCodeOrHash {
    pub fn code_hash(&self) -> [u8; HASH_LENGTH] {
        match &self {
            Self::CodeHash(data) => (*data).into(),
            Self::InlineCode(data) => keccak256(data).into(),
            Self::IsEmpty(()) => NULL_HASH.into(),
        }
    }
}

        ...
        Tracer::CallTracer => {
            ...
            if tracer.config.diff_mode {
                ...
                if call_frames.is_empty() {
                    ...
                    let frame = MonadCallFrame::from(call_frame);
                    Ok(Some(frame))
                }
            } else {
...
async fn build_call_tree(
    ...
    let mut log_index_counter = 0usize;

    let root = Rc::new(RefCell::new(MonadCallFrame::from(root)));

    // Build call tree and assign sequential log indices in execution order
    //
    // The input is a flat list of CallFrames ordered by execution (depth-first traversal).
    // We need to:
    // 1. Build a tree structure where child calls are nested under their parent frames
    // 2. Assign sequential log indices to all logs in the order they were emitted during execution
    //
    // Logs within a frame are split around child calls. For example:
    //   Frame A: [log0, log1, CALL to B, log2, log3]
    //   Frame B: [log4, log5]
    // The correct log index order is: log0, log1, log4, log5, log2, log3
    //
    // We maintain a stack of (frame, next_log_idx) tuples where:
    // - frame: A reference to the current frame being processed
    // - next_log_idx: Index of the next log in this frame that needs an index assigned
    //
    // Process frames in execution order:
    // - When we encounter a deeper frame (child call), we:
    //   1. Assign an index to the first log of the parent (if any) before descending
    //   2. Add the child to the parent's calls list
    //   3. Push the child onto the stack
    // - When we encounter a frame at the same or lower depth (sibling or returning):
    //   1. Assign indices to all remaining logs in the stack top frame
    //   2. Pop frames from the stack until we find the correct parent
    //   3. Push the new frame onto the stack
    // - After processing all frames, assign indices to logs in any remaining stack frames
    ...
    for value in nodes {
        ...
        loop {
            let Some((mut last, next_log_idx)) = stack
                .last_mut()
                .map(|(last, next_log_idx)| (last.borrow_mut(), next_log_idx))
            else {
            ...
            if last.depth < depth {
                // Case 1: New frame is a child call of the current stack top frame
                // Before descending into the child, assign an index to the first log of the parent
                // (if any exists). This log was emitted before the child call was made.
                if *next_log_idx == 0 && !last.logs.is_empty() {
                    last.logs[0].index = Quantity(log_index_counter as u64);
                    log_index_counter += 1;
                    *next_log_idx = 1;
                }
                // Add the child to parent's calls list and exit the loop to process it
                last.calls.push(Rc::clone(&new_node));
                break;
            }

            // Case 2: New frame is at the same or lower depth (sibling or returning up the call stack)
            // The current stack top frame is complete, so assign indices to all its remaining logs
            for log in last.logs[*next_log_idx..].iter_mut() {
                log.index = Quantity(log_index_counter as u64);
                log_index_counter += 1;
            }

            // Pop the completed frame and continue searching for the correct parent
            drop(last);
            stack.pop();
        }

        // Push the new frame onto the stack to process it next
        stack.push((new_node, 0));
    }

    // Assign indices to any remaining logs in frames still on the stack
    // This handles logs that were emitted after all child calls completed
    while let Some((top_ref, next_log_idx)) = stack.pop() {
        let mut top = top_ref.borrow_mut();
        for log in top.logs[next_log_idx..].iter_mut() {
            log.index = Quantity(log_index_counter as u64);
            log_index_counter += 1;
        }
    }
